...
for (( i=0; i<${#datasets[@]}; i++ )); do    
    python ./track_algorithm.py -a MCCT -n SuperFast -d ${datasets[$i]} -e ${super_fast_experts[@]} -f 0.1
done

fast_experts=("Ocean" "SiamBAN" "SiamCAR" "SiamFC++" "SiamRPN++")
for (( i=0; i<${#datasets[@]}; i++ )); do
        python ./track_algorithm.py -a MCCT -n Fast -d ${datasets[$i]} -e ${fast_experts[@]} -f 0.1
done

normal_experts=("DiMP" "DROL" "KYS" "PrDiMP" "RPT")
for (( i=0; i<${#datasets[@]}; i++ )); do
        python ./track_algorithm.py -a MCCT -m Normal -d ${datasets[$i]} -e ${normal_experts[@]} -f 0.1
done
