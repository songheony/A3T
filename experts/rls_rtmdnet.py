...
sys.path.append("external/RLS-RTMDNet/")
sys.path.append("external/RLS-RTMDNet/modules")
...
class RLS_RTMDNet(BaseTracker):
    def __init__(self):
        super(RLS_RTMDNet, self).__init__("RLS-RTMDNet")

        opts["model_path"] = path_config.RLS_RTMDNET_MODEL
        ...
        self.dtype = torch.cuda.FloatTensor  # run on GPU
        ...
        self.init_optimizer = set_optimizer(self.model, 0.01)
        self.update_optimizer = set_optimizer(self.model, opts["lr_update"])
        self.update_optimizer_owm = set_optimizer(self.model, 0.01)
    ...
    def initialize(self, image_file, box):
        ...
        padded_x1 = (
            neg_examples[:, 0] - neg_examples[:, 2] * (opts["padding"] - 1.0) / 2.0
        ).min()
        padded_y1 = (
            neg_examples[:, 1] - neg_examples[:, 3] * (opts["padding"] - 1.0) / 2.0
        ).min()
        padded_x2 = (
            neg_examples[:, 0] + neg_examples[:, 2] * (opts["padding"] + 1.0) / 2.0
        ).max()
        ...
        for bidx in range(0, scene_boxes.shape[0]):
            ...
            cur_pos_rois = Variable(
                torch.from_numpy(cur_pos_rois.astype("float32"))
            ).cuda()
            ...
            cur_neg_rois = Variable(
                torch.from_numpy(cur_neg_rois.astype("float32"))
            ).cuda()
            ...
            cur_bbreg_rois = Variable(
                torch.from_numpy(cur_bbreg_rois.astype("float32"))
            ).cuda()
            cur_bbreg_feats = self.model.roi_align_model(feat_map, cur_bbreg_rois)
            cur_bbreg_feats = cur_bbreg_feats.view(
                cur_bbreg_feats.size(0), -1
            ).data.clone()
        ...
        for iidx in range(replicateNum):
            ...
            extra_scene_box_center = extra_scene_box[0:2] + extra_scene_box[2:4] / 2.0
            extra_scene_box_size = extra_scene_box[2:4] * (opts["padding"] + 0.6)
            extra_scene_box[0:2] = extra_scene_box_center - extra_scene_box_size / 2.0
            ...
            cur_extra_neg_examples = gen_samples(
                SampleGenerator("uniform", (ishape[1], ishape[0]), 0.3, 2, 1.1),
                extra_target_bbox,
            ...
            if iidx == 0:
                ...
                extra_bbreg_examples = np.concatenate(
                    (extra_bbreg_examples, np.copy(cur_extra_bbreg_examples)), axis=0
                )

        extra_pos_rois = Variable(
            torch.from_numpy(extra_pos_rois.astype("float32"))
        ).cuda()
        extra_neg_rois = Variable(
            torch.from_numpy(extra_neg_rois.astype("float32"))
        ).cuda()
        # bbreg rois
        extra_bbreg_rois = Variable(
            torch.from_numpy(extra_bbreg_rois.astype("float32"))
        ).cuda()
        ...
        # bbreg feat
        extra_bbreg_feats = self.model.roi_align_model(
            extra_feat_maps, extra_bbreg_rois
        )
        extra_bbreg_feats = extra_bbreg_feats.view(
            extra_bbreg_feats.size(0), -1
        ).data.clone()
        ...
        self.P4 = torch.autograd.Variable(
            torch.eye(512 * 3 * 3 + 1).type(self.dtype), volatile=True
        )
        self.P5 = (
            torch.autograd.Variable(torch.eye(512 + 1).type(self.dtype), volatile=True)
            * 10
        )
        self.P6 = torch.autograd.Variable(
            torch.eye(512 + 1).type(self.dtype), volatile=True
        )

        self.W4 = torch.autograd.Variable(
            torch.zeros(512 * 3 * 3 + 1, 512).type(self.dtype), volatile=True
        )
        self.W5 = torch.autograd.Variable(
            torch.zeros(512 + 1, 512).type(self.dtype), volatile=True
        )
        self.W6 = torch.autograd.Variable(
            torch.zeros(512 + 1, 2).type(self.dtype), volatile=True
        )
        ...
        if pos_feats.size(0) > opts["n_pos_update"]:
            ...
            self.pos_feats_all = [
                pos_feats.index_select(
                    0, torch.from_numpy(pos_idx[0 : opts["n_pos_update"]]).cuda()
                )
            ]
        if neg_feats.size(0) > opts["n_neg_update"]:
            ...
            self.neg_feats_all = [
                neg_feats.index_select(
                    0, torch.from_numpy(neg_idx[0 : opts["n_neg_update"]]).cuda()
                )
            ]
    ...
    def track(self, image_file):
        ...
        padded_x1 = (
            samples[:, 0] - samples[:, 2] * (opts["padding"] - 1.0) / 2.0
        ).min()
        padded_y1 = (
            samples[:, 1] - samples[:, 3] * (opts["padding"] - 1.0) / 2.0
        ).min()
        padded_x2 = (
            samples[:, 0] + samples[:, 2] * (opts["padding"] + 1.0) / 2.0
        ).max()
        ...
        sample_rois[:, 0:2] -= np.repeat(
            np.reshape(padded_scene_box[0:2], (1, 2)), sample_rois.shape[0], axis=0
        )
        ...
        if success:
            ...
            for bidx in range(0, scene_boxes.shape[0]):
                ...
                cur_pos_feats = self.model.roi_align_model(feat_map, cur_pos_rois)
                cur_pos_feats = cur_pos_feats.view(
                    cur_pos_feats.size(0), -1
                ).data.clone()
                ...
                cur_neg_feats = self.model.roi_align_model(feat_map, cur_neg_rois)
                cur_neg_feats = cur_neg_feats.view(
                    cur_neg_feats.size(0), -1
                ).data.clone()
        ...
        if not success:
            ...
            pos_data = torch.stack(self.pos_feats_all[-nframes:], 0).view(
                -1, self.feat_dim
            )
...
def train_owm(
    ...
    for iter in range(maxiter):
        ...
        score, h_list = model.forward_owm(
            torch.cat((batch_pos_feats, batch_neg_feats), dim=0), in_layer=in_layer
        )
        ...
        def pro_weight(p, x, w, w_bk, alpha=1.0):
            ...
            w_tmp = (
                torch.mm(
                    torch.cat((w_bk.grad.data, torch.t(w.grad.data.repeat(1, 1))), 1),
                    torch.t(p.data),
                )
                * gama
            )
        ...
        for n, w in model.named_parameters():
            ...
            if n == "layers.fc5.1.bias":
                pro_weight(P5, h_list[1], w, w_bk, alpha=alpha_array[1])

            if n == "branches.0.1.weight":
                w_bk = w

            if n == "branches.0.1.bias":
                pro_weight(P6, h_list[2], w, w_bk, alpha=alpha_array[2])
...
def train_original(
    model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer="fc4"
):
    model.train()
