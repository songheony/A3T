...
baselines=("Random" "WithoutOffline" "WithoutDelay")

super_fast_experts=("DaSiamRPN" "SiamDW" "SiamRPN" "SPM")
for (( i=0; i<${#datasets[@]}; i++ )); do    
    for (( j=0; j<${#baselines[@]}; j++ )); do
        python ./track_algorithm.py -a ${baselines[$j]} -m SuperFast -d ${datasets[$i]} -e ${super_fast_experts[@]} -f 11
    done
done
...
# for (( i=0; i<${#datasets[@]}; i++ )); do    
#     for (( j=0; j<${#baselines[@]}; j++ )); do
#         python ./track_algorithm.py -a ${baselines[$j]} -m Fast -d ${datasets[$i]} -e ${fast_experts[@]} -f 11
#     done
# done
...
# for (( i=0; i<${#datasets[@]}; i++ )); do    
#     for (( j=0; j<${#baselines[@]}; j++ )); do
#         python ./track_algorithm.py -a ${baselines[$j]} -m Normal -d ${datasets[$i]} -e ${normal_experts[@]} -f 11
#     done
# done
...
# for (( i=0; i<${#datasets[@]}; i++ )); do    
#     for (( j=0; j<${#baselines[@]}; j++ )); do
#         python ./track_algorithm.py -a ${baselines[$j]} -m SiamDW -d ${datasets[$i]} -e ${siamdw_experts[@]} -f 11
#     done
# done
...
# for (( i=0; i<${#datasets[@]}; i++ )); do    
#         python ./track_algorithm.py -a ${baselines[$j]} -m SiamRPN++ -d ${datasets[$i]} -e ${siamrpn_experts[@]} -f 11
#     done
# done