...
for (( i=0; i<${#datasets[@]}; i++ )); do    
    python ./track_algorithm.py -a HDT -m SuperFast -d ${datasets[$i]} -e ${super_fast_experts[@]} -t 0.65
done

fast_experts=("Ocean" "SiamBAN" "SiamCAR" "SiamFC++" "SiamRPN++")
for (( i=0; i<${#datasets[@]}; i++ )); do
        python ./track_algorithm.py -a HDT -m Fast -d ${datasets[$i]} -e ${fast_experts[@]} -t 0.65
done

normal_experts=("DiMP" "DROL" "KYS" "PrDiMP" "RPT")
for (( i=0; i<${#datasets[@]}; i++ )); do
        python ./track_algorithm.py -a HDT -m Normal -d ${datasets[$i]} -e ${normal_experts[@]} -t 0.65
done
...
for (( i=0; i<${#datasets[@]}; i++ )); do
        python ./track_algorithm.py -a HDT -m SiamDW -d ${datasets[$i]} -e ${siamdw_experts[@]} -t 0.65
done
...
for (( i=0; i<${#datasets[@]}; i++ )); do
        python ./track_algorithm.py -a HDT -m SiamRPN++ -d ${datasets[$i]} -e ${siamrpn_experts[@]} -t 0.65
done