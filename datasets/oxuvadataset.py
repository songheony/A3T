...
class Task(object):
    ...
    def __init__(
        self, init_time, init_rect, labels=None, last_time=None, attributes=None
    ):
...
class OxUvADatasetClass(BaseDataset):
    ...
    def __init__(self):
        ...
        self.imfile = lambda vid, t: os.path.join(
            self.base_path, "images", "test", vid, "{:06d}.jpeg".format(t)
        )

    def get_sequence_list(self):
        return SequenceList(
            [self._construct_sequence(key, task) for key, task in self.tasks.items()]
        )
    ...
    def _construct_sequence(self, key, task):
        ...
        frames_list = [
            self.imfile(vid, t) for t in range(task.init_time, task.last_time + 1)
        ]
